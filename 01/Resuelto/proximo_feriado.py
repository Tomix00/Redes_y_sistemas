...
class NextHoliday:
    ...
    def next_holidays_byType(self, tipo):
        response = requests.get(get_url(self.year))
        ...
        holiday = list(h for h in holidays
                       if ((h['mes'] == today['month'] and
                            h['dia'] > today['day']) or
    ...
    def string_holiday(self):
        return (f"{day_of_week(self.holiday['dia'],
                               self.holiday['mes'],
                               self.year)} "
                f"{self.holiday['dia']} de {months[self.holiday['mes']]} del "
                f"{self.year}")

    def string_motivo(self):
        return self.holiday['motivo']
